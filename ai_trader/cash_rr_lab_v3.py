"""Risk-Reward Lab for confirmed Cash Ensemble V3 previews.

Reads V3 evidence plus exact EA preview receipts and evaluates the same ten
...
def stop(code):
    print(f"V3_RR_STOP | {code} | NO_API | NO_ORDER")
    raise SystemExit(1)


def load_confirmed():
    if not DB.is_file():stop("V3_LEDGER_NOT_FOUND")
    ...
    for bar,sid,local_text,result_text in rows:
        ...
        if final in {"BUY","SELL"} and result.get("delivery")=="READY_FOR_EA":
            ...
            out.append({
                ...
                "buy_votes":int(local.get("buy_votes",0)),
                "sell_votes":int(local.get("sell_votes",0)),
                "confidence":ai.get("confidence"),"regime":ai.get("market_regime"),
            })
    return out
...
def main():
    ...
    if not confirmed:
        print("V3_RR | CONFIRMED_DIRECTIONAL=0 | NO_API | NO_ORDER");return 0
    ...
    try:
        ...
        for item in confirmed:
            ...
            try:
                ...
                print(f"V3_RR_SIGNAL | bar={item['bar']} | PLAN_REJECT | {exc}")
                skipped+=1;continue

            states,tick_count,_=rr.evaluate_receipt(mt5,receipt,targets,end_ms)
            exact+=1
            print(
                f"V3_RR_SIGNAL | bar={item['bar']} | action={item['action']} | "
                f"votes={item['buy_votes']}B/{item['sell_votes']}S | "
                f"ai_conf={item['confidence']} | regime={item['regime']} | "
                f"suggested_rr={item['suggested_rr']:.2f}R | ticks={tick_count}"
            )
            for ratio in rr.RATIOS:
                ...
                print(f"V3_RR | {ratio:.2f}R | TP={tp:.8f} | {st}{marker}")
        ...
        print(f"V3_RR_SUMMARY | EXACT={exact} | SKIPPED={skipped}")
        for ratio in rr.RATIOS:
            ...
            print(
                f"V3_RR_METRIC | {ratio:.2f}R | TP_FIRST={a['tp']} | "
                f"SL_FIRST={a['sl']} | UNRESOLVED={a['unresolved']} | "
                f"win_rate={wr_text} | expectancy={ex_text}"
            )
        print(
            f"V3_SUGGESTED_RR | COUNT={suggested['count']} | TP_FIRST={suggested['tp']} | "
            f"SL_FIRST={suggested['sl']} | UNRESOLVED={suggested['unresolved']}"
        )
        print("V3_RR_NOTE | research-only; small samples do not prove live profitability")
        print("V3_RR_DONE | NO_API | NO_ORDER | NO_LEDGER_WRITE")
        return 0
    finally:
        mt5.shutdown()
