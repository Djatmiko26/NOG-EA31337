"""Cash Ensemble V3: 10 local logics -> independent AI confirmation -> PREVIEW.

Research-only safety:
- separate ledger: data/cash_ensemble_v3.sqlite3
...
- AI must independently agree with local direction and meet confidence threshold
...
import ensemble_logic as ensemble

ROOT=Path(__file__).resolve().parent
DB=ROOT/"data"/"cash_ensemble_v3.sqlite3"
HOST,PORT="127.0.0.1",8765
VERSION="cash-ensemble-v3"
EVENT_CAP=40
API_CAP=12
...
def implementation():
    names=("cash_ensemble_v3.py","ensemble_logic.py","cash_demo.py",
...
class Ledger:
    ...
    def __init__(self,path:Path,spec:dict):
        ...
        try:
            ...
            frozen=encode({**spec,"version":VERSION,"event_cap":EVENT_CAP,
                           "api_cap":API_CAP,"ai_min_confidence":AI_MIN_CONFIDENCE,
            ...
            if saved!=frozen:raise ValueError("V3_FROZEN_SETTINGS_CHANGED_PRESERVE_LEDGER")
    ...
    def reserve(self,bar:int,decision:ensemble.EnsembleDecision,payload:dict):
        if type(bar) is not int or bar<=0:raise ValueError("V3_INVALID_BAR")
    ...
    def mark_api(self,bar:int):
        with self.db:
            ...
            if cur.rowcount!=1:raise ValueError("V3_API_RESERVATION_FAILED")

    def finish(self,bar:int,result:dict,status="SUCCESS"):
        if status not in {"SUCCESS","ERROR","REFUSED","INVALID","INCOMPLETE"}:
            raise ValueError("V3_INVALID_STATUS")
        with self.db:
            cur=self.db.execute(
                "UPDATE events SET status=?,result=? WHERE bar=? AND status='STARTED'",
                (status,encode(result),bar)
            )
            if cur.rowcount!=1:raise ValueError("V3_UNRESERVED_OR_FINISHED_EVENT")


def status():
    if not DB.is_file():
        log(f"V3_LEDGER_NOT_STARTED | EVENTS=0/{EVENT_CAP} | API=0/{API_CAP} | API_CALLS=0")
        return
    ...
    try:
        ...
        for bar,st,used,local_text,result_text in rows:
            ...
            log(
                f"V3 | bar={bar} | {st} | local={local.get('action','-')} | "
                f"votes={local.get('buy_votes',0)}B/{local.get('sell_votes',0)}S | "
                f"api={used} | ai={ai.get('action','-')}:{ai.get('confidence','-')} | "
                f"final={final} | delivery={result.get('delivery','-')}"
            )
        log(
            f"V3_STATUS | EVENTS={len(rows)}/{EVENT_CAP} | API={api}/{API_CAP} | "
...
def _log_decision(decision):
    log(
        f"V3_ENSEMBLE | action={decision.action} | BUY={decision.buy_votes} | "
        f"SELL={decision.sell_votes} | WAIT={decision.wait_votes} | VETO={decision.vetoes} | "
        f"core={decision.core_support} | consensus={decision.consensus:.3f} | "
        f"rr_suggest={decision.suggested_rr:.2f}R | {decision.reason}"
    )
    for v in decision.votes:
        log(f"V3_VOTE | {v.name} | {v.signal} | strength={v.strength:.2f} | {v.reason}")
...
def process_event(ledger,state,client,core,feed,account,before,payload,decision,event_m,event_w):
    ...
    elif ai["confidence"]<AI_MIN_CONFIDENCE:reason="AI_LOW_CONFIDENCE";final="WAIT"
...
def run_one(feed,core):
    ...
    try:
        ...
        if ledger.blocked():raise ValueError("V3_UNRESOLVED_EVENT_USE_STATUS")
        ...
        log(
            f"V3_READY | 10_LOGICS | PREVIEW_ONLY | events={ledger.event_count()}/{EVENT_CAP} | "
        ...
        while ledger.event_count()==initial:
            ...
            try:
                ...
                if not state.receiver_ready(m):
                    gate.reset();state.clear();message="V3_WAIT_CASH_EA"
                    ...
                    log(
                        f"V3_RESULT | status={result.get('status','-')} | "
